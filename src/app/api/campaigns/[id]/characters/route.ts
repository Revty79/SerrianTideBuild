...
export async function POST(
  ...
  try {
    ...
    // Validate player link belongs to this campaign first.
    ...
    if (!campaignPlayer) {
      return NextResponse.json({ ok: false, error: "PLAYER_NOT_IN_CAMPAIGN" }, { status: 400 });
    }

    // Check if user has permission (G.O.D or the player creating their own character)
    const isGod = campaign.createdBy === user.id;

    if (!isGod) {
      if (campaignPlayer.userId !== user.id) {
        return NextResponse.json({ ok: false, error: "FORBIDDEN" }, { status: 403 });
      }
    }
...
export async function PUT(
  ...
  try {
    ...
    const [characterInCampaign] = await db
      .select({ id: schema.campaignCharacters.id })
      .from(schema.campaignCharacters)
      .innerJoin(
        schema.campaignPlayers,
        eq(schema.campaignCharacters.campaignPlayerId, schema.campaignPlayers.id)
      ...
      .where(
        and(
          eq(schema.campaignCharacters.id, body.characterId),
          eq(schema.campaignPlayers.campaignId, campaignId)
        )
      )
      .limit(1);
...
export async function DELETE(
  ...
  try {
    ...
    const [characterInCampaign] = await db
      ...
      .where(
        and(
          eq(schema.campaignCharacters.id, characterId),
          eq(schema.campaignPlayers.campaignId, campaignId)
        )
      )
      .limit(1);
